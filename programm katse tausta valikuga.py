...
def loo_muster():
    ...
    tausta_valik = kas_taust.get()
    
    if valitud_tüüp.get() == 1:
        l.svg_fail(kunstityyp, int(laius), int(kõrgus), failinimi, tausta_valik)
    else:
        l.png_fail(kunstityyp, int(laius), int(kõrgus), failinimi, tausta_valik)
...
tausta_nupu_rida, tausta_nupu_veerg,tausta_nupu_ulatus = 1, 2, 2
...
# tausta nupp
kas_taust = IntVar()
nupp_taust = Checkbutton(raam, text = 'taustaga', variable = kas_taust, onvalue = 1, offvalue = 0, selectcolor = tausta_värv, highlightbackground = tausta_värv, activebackground = tausta_värv, activeforeground = teksti_värv, fg = teksti_värv, bg = tausta_värv)
nupp_taust.grid(row = tausta_nupu_rida, column = tausta_nupu_veerg, columnspan = tausta_nupu_ulatus)
