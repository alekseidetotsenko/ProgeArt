...
# menüü nupp on veider, kuna liigub paigast. et seda ei juhtuks, on parem teha kast kuhu sisse ta panna.
menüü_raam = Frame(raam) # neid oleks kõigi nuppude paigutamiseks vaja
menüü_raam.grid(row = 5, column = 1, columnspan = 2, sticky = W + E)
...
menüü.pack(in_ = menüü_raam, fill = BOTH)
...
nupp_tagasi.grid(row = tagasi_nupu_rida, column = tagasi_nupu_veerg, sticky = W)
nupp_loo_muster = Button(raam, text='Loo muster', command = loo_muster, fg="white", bg="green")
...
silt_faili_tüüp = Label(raam, text = 'Tüüp:')
silt_faili_tüüp.grid(row = tüübi_sildi_rida, column = tüübi_sildi_veerg, sticky = W)

valitud_tüüp = IntVar()
nupp_png = Radiobutton(raam, text = '.png', value = '.png', variable = valitud_tüüp) # siia on kuidagi vaja lisada see: lambda: png_fail(c, laius, kõrgus)
nupp_png.grid(row = png_nupu_rida, column = png_nupu_veerg)
nupp_svg = Radiobutton(raam, text = '.svg', value = '.svg', variable = valitud_tüüp) # ja siia see: lambda: svg_fail(c, laius, kõrgus)
nupp_svg.grid(row = svg_nupu_rida, column = svg_nupu_veerg)

# faili nime sisestamise osa
silt_faili_nimi = Label(raam, text = 'Nimi:')
silt_faili_nimi.grid(row = nime_sildi_rida, column = nime_sildi_veerg, sticky = W)
kast_faili_nimi = Entry(raam)
